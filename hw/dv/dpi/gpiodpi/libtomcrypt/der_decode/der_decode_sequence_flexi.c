...
static int s_new_element(ltc_asn1_list **l)
{
   /* alloc new link */
   if (*l == NULL) {
      *l = (ltc_asn1_list*)XCALLOC(1, sizeof(ltc_asn1_list));
      if (*l == NULL) {
         return CRYPT_MEM;
      }
   } else {
      (*l)->next = (ltc_asn1_list_*)XCALLOC(1, sizeof(ltc_asn1_list));
      if ((*l)->next == NULL) {
         return CRYPT_MEM;
      }
      (*l)->next->prev = *l;
     ...
     /* now switch on type */
      switch (identifier) {
         case 0x01: /* BOOLEAN */
            ...
            if ((err = der_decode_boolean(in, *inlen, (int*)l->data)) != CRYPT_OK) {
               goto error;
            }
         ...
         case 0x03: /* BIT */
            ...
            if ((err = der_decode_bit_string(in, *inlen,(unsigned char*) l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }
         ...
         case 0x04: /* OCTET */
            ...
            if ((err = der_decode_octet_string(in, *inlen, (unsigned char*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }
         ...
         case 0x06: /* OID */
            ...
            if ((err = der_decode_object_identifier(in, *inlen, (long unsigned int*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_object_identifier((const long unsigned int*)l->data, l->size, &len)) != CRYPT_OK) {
               goto error;
            }
         ...
         case 0x0C: /* UTF8 */
            ...
            if ((err = der_decode_utf8_string(in, *inlen, (wchar_t*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_utf8_string((const wchar_t*)l->data, l->size, &len)) != CRYPT_OK) {
               goto error;
            }
            break;
         ...
         case 0x13: /* PRINTABLE */
            ...
            if ((err = der_decode_printable_string(in, *inlen, (unsigned char*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_printable_string((const unsigned char*)l->data, l->size, &len)) != CRYPT_OK) {
               goto error;
            }
            break;
         ...
         case 0x14: /* TELETEXT */
            ...
            if ((err = der_decode_teletex_string(in, *inlen, (unsigned char*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_teletex_string((const unsigned char*)l->data, l->size, &len)) != CRYPT_OK) {
               goto error;
            }
            break;
         ...
         case 0x16: /* IA5 */
            ...
            if ((err = der_decode_ia5_string(in, *inlen, (unsigned char*)l->data, &l->size)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_ia5_string((const unsigned char*)l->data, l->size, &len)) != CRYPT_OK) {
               goto error;
            }
            break;
         ...
         case 0x17: /* UTC TIME */
            ...
            len = *inlen;
            if ((err = der_decode_utctime(in, &len,(ltc_utctime*) l->data)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_utctime((const ltc_utctime*)l->data, &len)) != CRYPT_OK) {
               goto error;
            }
            break;
         ...
         case 0x18:
            ...
            if ((err = der_decode_generalizedtime(in, &len,(ltc_generalizedtime*) l->data)) != CRYPT_OK) {
               goto error;
            }

            if ((err = der_length_generalizedtime((const ltc_generalizedtime*)l->data, &len)) != CRYPT_OK) {
               goto error;
            }
