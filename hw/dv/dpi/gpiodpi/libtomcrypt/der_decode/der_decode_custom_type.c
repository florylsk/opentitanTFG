                                    ...
                                    unsigned int   flags)
{
   ...
   if (root == NULL) {
      ...
      if (root->pc == LTC_ASN1_PC_PRIMITIVE) {
         in_new = (unsigned char*)XMALLOC(inlen);
         if (in_new == NULL) {
            return CRYPT_MEM;
         }
   ...
   for (i = 0; i < (int)outlen; i++) {
       ...
       switch (type) {
           ...
           case LTC_ASN1_SHORT_INTEGER:
               z = inlen;
               if ((err = der_decode_short_integer(in + x, z, (long unsigned int*)data)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
           ...
           case LTC_ASN1_BIT_STRING:
               z = inlen;
               if ((err = der_decode_bit_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
           ...
           case LTC_ASN1_RAW_BIT_STRING:
               z = inlen;
               if ((err = der_decode_raw_bit_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
           ...
           case LTC_ASN1_OCTET_STRING:
               z = inlen;
               if ((err = der_decode_octet_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
           ...
           case LTC_ASN1_OBJECT_IDENTIFIER:
               z = inlen;
               if ((err = der_decode_object_identifier(in + x, z, (long unsigned int*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
               if ((err = der_length_object_identifier((const long unsigned int*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_TELETEX_STRING:
               z = inlen;
               if ((err = der_decode_teletex_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
               if ((err = der_length_teletex_string((const unsigned char*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_IA5_STRING:
               z = inlen;
               if ((err = der_decode_ia5_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
               if ((err = der_length_ia5_string((const unsigned char*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_PRINTABLE_STRING:
               z = inlen;
               if ((err = der_decode_printable_string(in + x, z, (unsigned char*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
               if ((err = der_length_printable_string((const unsigned char*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_UTF8_STRING:
               z = inlen;
               if ((err = der_decode_utf8_string(in + x, z, (wchar_t*)data, &size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               list[i].size = size;
               if ((err = der_length_utf8_string((const wchar_t*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_UTCTIME:
               z = inlen;
               if ((err = der_decode_utctime(in + x, &z, (ltc_utctime*)data)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_GENERALIZEDTIME:
               z = inlen;
               if ((err = der_decode_generalizedtime(in + x, &z, (ltc_generalizedtime*)data)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_SET:
               z = inlen;
               if ((err = der_decode_set(in + x, z, (ltc_asn1_list*)data, size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               if ((err = der_length_sequence((const ltc_asn1_list*)data, size, &z)) != CRYPT_OK) {
                  goto LBL_ERR;
               }
               break;
           ...
           case LTC_ASN1_SEQUENCE:
               ...
               z = inlen;
               err = der_decode_sequence_ex(in + x, z, (ltc_asn1_list*)data, size, flags);
           ...
           case LTC_ASN1_CHOICE:
               z = inlen;
               if ((err = der_decode_choice(in + x, &z, (ltc_asn1_list*)data, size)) != CRYPT_OK) {
                  if (!ordered || list[i].optional) { continue; }
                  goto LBL_ERR;
               }
               break;
