...
with open(readPipeLoc,"r") as pipe:
    ...
    while True:
        ...
        if firstChar=="\x00" and secondChar=="\x00":
            p.success()
            log.success("Process finished, reconstructed path: "+str(geohashes))
            break
...
log.success("Successfully written the CSV file with the path's reconstructed coordinates to"+tmpCsvFileName)